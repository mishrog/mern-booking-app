...
const API_BASE_URL = import.meta.env.VITE_API_BASE_URL || ""; // if there is no api base url, just use the same server for all the req
...
export const register = async (formData: RegisterFormData) => {
  const response = await fetch(`${API_BASE_URL}/api/users/register`, {
    method: "POST",
    credentials: "include", // include any http cookies along with the request and also set any cookies we get back from the server
    headers: {
      "Content-Type": "application/json", // tells server to expect body of req to be in json format
    },
  ...
  if (!response.ok) {
    throw new Error(responseBody.message); // response comes back with a message property
  }
};
